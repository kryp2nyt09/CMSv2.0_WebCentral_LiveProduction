...
namespace CMSVersion2.Maintenance.CMSMaintenance.UserModal.OrganizationType
{
    public partial class EditOrganizationType : System.Web.UI.Page
	{
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                else
                {
                    ...
                    DataTable OrganizationType = GetOrganizationType(new Guid(ID));
                    int counter = 0;
                    foreach (DataRow row in OrganizationType.Rows)
                    {
                        if (counter == 0)
                        {
                            string x = row["OrganizationTypeId"].ToString();
                            string y = row["OrganizationTypeName"].ToString();
        ...
        private DataTable GetOrganizationType(Guid ID)
        {

            //DataTable data = new DataTable();
            DataSet data = BLL.OrganizationType.GetOrganizationTypeById(ID, getConstr.ConStrCMS);
        ...
        {
            base.OnInit(e);
            this.Page.Title = "Edit Organization Type";
        }
