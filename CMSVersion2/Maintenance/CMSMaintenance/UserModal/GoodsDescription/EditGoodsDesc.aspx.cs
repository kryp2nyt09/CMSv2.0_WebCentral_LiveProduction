...
namespace CMSVersion2.Maintenance.CMSMaintenance.UserModal.GoodsDescription
{
    public partial class EditGoodsDesc : System.Web.UI.Page
	{
            ...
            if (!IsPostBack)
            {
                ...
                else
                {
                    ...
                    foreach (DataRow row in GroupInfo.Rows)
                    {
                        if (counter == 0)
                        {
                            string x = row["GoodsDescriptionId"].ToString();
                            string y = row["GoodsDescriptionName"].ToString();
        ...
        public DataTable GetGroupIsland(Guid ID)
        {
            ...
            DataSet data = BLL.GoodsDescription.GetGoodsDescriptionById(ID, getConstr.ConStrCMS);
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            ...
            Guid GoodsID = new Guid(lblGroupID.Text);
            BLL.GoodsDescription.UpdateGoodsDesc(GoodsID, ID, txtIslandGroup.Text, 1, getConstr.ConStrCMS);
