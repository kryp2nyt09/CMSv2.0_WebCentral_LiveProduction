...
namespace CMSVersion2.Maintenance.CMSMaintenance.UserModal.RegionMaintenance
{
    public partial class EditRegion : System.Web.UI.Page
	{
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                else
                {
                    ...
                    string RegionId = Request.QueryString["ID"].ToString();
                    DataTable GroupInfo = GetRegionDetails(new Guid(RegionId));
                    int counter = 0;
        ...
        {
            rcbGroup.DataSource = BLL.IslandGroup.GetIslandGroup(getConstr.ConStrCMS);
            rcbGroup.DataValueField = "GroupId";
            rcbGroup.DataTextField = "GroupName";
            rcbGroup.DataBind();
        }
        ...
        {
            base.OnInit(e);
            this.Page.Title = "Edit Region";
        }

        public DataTable GetRegionDetails(Guid ID)
        {
            //DataTable data = new DataTable();
            DataSet data = BLL.Region.GetRegionById(ID, getConstr.ConStrCMS);
        ...
        {
            Guid GroupID = new Guid(rcbGroup.SelectedItem.Value.ToString());
            Guid RegionId = new Guid(lblGroupID.Text);
            Guid ModifiedBy = new Guid("11111111-1111-1111-1111-111111111111");
            string host = HttpContext.Current.Request.Url.Authority;
            string RegionName = txtRegionName.Text;
            BLL.Region.UpdateRegion(RegionId, GroupID, ModifiedBy, RegionName, 1, getConstr.ConStrCMS);
