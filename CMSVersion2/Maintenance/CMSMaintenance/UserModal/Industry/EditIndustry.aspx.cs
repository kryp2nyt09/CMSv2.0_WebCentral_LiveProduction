...
namespace CMSVersion2.Maintenance.CMSMaintenance.UserModal.Industry
{
    public partial class EditIndustry : System.Web.UI.Page
	{
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                else
                {
                    ...
                    DataTable Industry = GetIndustry(new Guid(ID));
                    int counter = 0;
                    foreach (DataRow row in Industry.Rows)
                    {
                        if (counter == 0)
                        {
                            string x = row["IndustryId"].ToString();
                            string y = row["IndustryName"].ToString();
        ...
        private DataTable GetIndustry(Guid ID)
        {

            //DataTable data = new DataTable();
            DataSet data = BLL.Industry.GetIndsutryById(ID, getConstr.ConStrCMS);
