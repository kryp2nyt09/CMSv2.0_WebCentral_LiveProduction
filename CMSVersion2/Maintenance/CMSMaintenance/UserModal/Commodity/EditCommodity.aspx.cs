    ...
    public partial class EditCommodity : System.Web.UI.Page
	{
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                {
                    string CommodityId = Request.QueryString["ID"].ToString();
                    DataTable GroupInfo = getCommodityDetails(new Guid(CommodityId));
                    int counter = 0;
                    foreach (DataRow row in GroupInfo.Rows)
                    {
                        if (counter == 0)
                        {
                            ...
                            RadComboBoxItem item = rcbGroup.FindItemByText(CommodityTypeName);
                            item.Selected = true;

                            txtRegionName.Text = CommodityName;
                            lblGroupID.Text = Id;
        ...
        {
            base.OnInit(e);
            this.Page.Title = "Edit Commodity";
        }
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            ...
            BLL.Commodity.UpdateCommodity(CommodityId, CommodityName, CommodityTypeId, ModifiedBy, getConstr.ConStrCMS);
