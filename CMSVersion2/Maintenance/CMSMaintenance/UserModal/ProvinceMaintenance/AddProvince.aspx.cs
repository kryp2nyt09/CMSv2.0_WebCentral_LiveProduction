    ...
    public partial class AddProvince : System.Web.UI.Page
	{
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                //        DataTable GroupInfo = GetProvinceById(new Guid(RegionId));
                //        int counter = 0;
                ...
                //                string Id = row["ProvinceId"].ToString();
                //                string ProvinceName = row["ProvinceName"].ToString();
                //                string Region_Id = row["RegionId"].ToString();
                //                string RegionName = row["RegionName"].ToString();

                //                //Use RadComboBoxItem.Selected
                //                RadComboBoxItem item = rcbGroup.FindItemByText(RegionName);
                //                item.Selected = true;

                //                txtRegionName.Text = ProvinceName;
                //                lblGroupID.Text = Id;
        ...
        {
            rcbGroup.DataSource = BLL.Region.GetRegion(getConstr.ConStrCMS);
            rcbGroup.DataValueField = "RegionId";
            rcbGroup.DataTextField = "RegionName";
            rcbGroup.DataBind();
        }
        ...
        {
            Guid RegionId = new Guid(rcbGroup.SelectedItem.Value.ToString());
            ...
            BLL.Province.InsertProvince(RegionId, ModifiedBy, RegionName, 1, getConstr.ConStrCMS);
