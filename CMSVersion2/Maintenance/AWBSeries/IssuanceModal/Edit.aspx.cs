    ...
    public partial class Edit : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                LoadArea();
                LoadEmployee();
                if (Request.QueryString["AwbIssuanceId"] == null)
                {
                    ...
                    foreach (DataRow row in Data.Rows)
                    {
                        if (counter == 0)
                        {
                            try
                            {
                                ...
                                rdDateAssigned.SelectedDate = Convert.ToDateTime(issuedDate);

                                txtStartSeries.Text = row["SeriesStart"].ToString();
                                txtEndSeries.Text = row["SeriesEnd"].ToString();
                                lblAwbIssuanceID.Text = row["AwbIssuanceId"].ToString();
        ...
        public DataTable GetAwbIssuanceInfor(Guid ID)
        {
            DataSet data = BLL.AwbIssuance.GetawbissuanceById(ID, getConstr.ConStrCMS);
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            ...
            BLL.AwbIssuance.updateAWBIssuanceWB(new Guid(lblAwbIssuanceID.Text), seriesStart, seriesEnd, assignedate, AreaId, BCOid, empId, CreatedBy, getConstr.ConStrCMS);
